...
@class MCButtonGroupBar;

/**
 *  「MCButtonGroupBarDelegate」协议定义的方法是与「MCButtonGroupBar」对象进行交互的
 */
@protocol MCButtonGroupBarDelegate <NSObject>
 ...
 */
- (void)buttonGroup:(nonnull MCButtonGroupBar *)buttonGroupBar
     didClickButton:(nonnull UIButton *)sender
     formGroupIndex:(NSUInteger)index;

@end

@interface MCButtonGroupBar : UIView
...
/** 这个对象充当按钮组栏的委托 */
@property (weak, nonatomic, nullable  ) id<MCButtonGroupBarDelegate> delegate;
 ...
 *  @return 如果初始化成功，将是一个「MCButtonGroupBar」对象，否则为 nil
 */
- (_Nonnull instancetype)initWithFrame:(CGRect)frame buttons:(NSArray * _Nonnull)buttons delegate:(id<MCButtonGroupBarDelegate> _Nullable)delegate;
